...
#define USE_HILITE_RUBBERBAND FALSE

/* color for normal (non-hilited or -selected locations */
#define CITY_COLOR_RGBA 0xf0f020e0
#define CITY_OUTLINE_COLOR_RGBA 0x00000050
#define CITY_SELECTED_COLOR_RGBA 0xe00000ff
#define CITY_SELECTED_OUTLINE_COLOR_RGBA 0xf0000080
...
void resize_canvas(GtkWidget *widget, GtkAllocation *alloc)
{
  if (alloc->width / 200.0 > alloc->height / 100.0)
    gnome_canvas_set_pixels_per_unit(map->canvas, (gdouble) ((alloc->width - 1.0) / 200));
  else
    gnome_canvas_set_pixels_per_unit(map->canvas, (gdouble) ((alloc->height - 1.0) / 100.0));
}
...
find_nearest ( double longitude, double latitude )
{
	...
	if (!gnome_map_is_loc_in_view (map, 
...
set_flag_marker (GnomeCanvasItem **curmarker, gchar *color,
		 double x, double y, double *oldx, double *oldy, gboolean selected)
{
	...
	group = gnome_canvas_item_new ( canvas_root,
        ...
        setup_item (gnome_canvas_item_new (GNOME_CANVAS_GROUP (group),
             ...
             "y1", -2.0,
             "y2", 2.0,
             "fill_color_rgba", 0xffffff70,
             "outline_color_rgba", selected ? CITY_SELECTED_OUTLINE_COLOR_RGBA :
                                              CITY_OUTLINE_COLOR_RGBA,
             "width_pixels", 1,
					   NULL));
...
map_mark_location_selected (GnomeMap *map, gint index)
{
    ...
    gnome_map_xlat_map2screen ( map, 
				loc->longitude,	loc->latitude,
				&selx, &sely );
    set_flag_marker (&selmarker, CITY_SELECTED_COLOR_RGBA,
		     selx, sely, &oldselx, &oldsely, TRUE);
    
    if (curselection >= 0) {
	gnome_canvas_item_set (markers[curselection],
			       "fill_color_rgba", CITY_COLOR_RGBA, NULL);
	gnome_canvas_item_show (markers[curselection]);
    }
      gnome_canvas_item_set (markers[index], "fill_color_rgba",
  			   CITY_SELECTED_COLOR_RGBA, NULL);
...
set_selection (gint index, gboolean jumpto)
{
    ...
    map_mark_location_selected (map, index);

    list_mark_location_selected (map, index, jumpto);
...
set_hilited (gint index, double item_x, double item_y)
{
	...
	GnomeCanvasGroup *canvas_root = gnome_canvas_root (GNOME_CANVAS (map->canvas));
	...
	if (hiliteindex >= 0 && hiliteindex != index) {
		...
		gnome_canvas_item_set ( markers[hiliteindex], 
					"fill_color_rgba", CITY_COLOR_RGBA, 
					NULL);
	}
	...
	set_flag_marker ( &hilitemarker, HILITED_COLOR,
			  points->coords[2], points->coords[3],
			  &oldhilitex, &oldhilitey, FALSE );
	
	
  	gnome_canvas_item_set ( markers[index],
  				"fill_color_rgba", 0x20ff20ff,
  				NULL);
...
/*  	    gnome_canvas_item_hide (hilitemarker); */
	...
	if (hiliteindex >= 0 && hiliteindex != curselection)
      ...
      if (hilitemarker) gnome_canvas_item_hide(hilitemarker);
...
item_event (GnomeCanvasItem *item, GdkEvent *event, gpointer data)
{
    ...
    gnome_canvas_item_w2i (map->image_item, &item_x, &item_y);
	...
	switch (event->button.button) {
	  case 1:
	    ...
	    gnome_map_xlat_screen2map ( map, item_x, item_y,
					&longitude, &latitude );
...
#define RAD 0.3
    ...
    item = gnome_canvas_item_new (canvas_root,
				  ...
				  "y2", y+RAD,
				  "fill_color_rgba", CITY_COLOR_RGBA,
          "outline_color_rgba", CITY_OUTLINE_COLOR_RGBA,
          "width_pixels", 1,
				  NULL);
#else
    item = gnome_canvas_item_new (canvas_root,
...
view_menu_activate (GtkWidget *widget, void *data)
{
    ...
    gnome_map_set_view (map, long1, lat1, long2, lat2);
...
create_location_list ( GtkWidget **returnlist )
{
	...
	*returnlist = gtk_clist_new_with_titles (1, titles);
    ...
    list_mark_location_selected(map, curselection, TRUE);
...
*/
    ...
    mainwindow = tool_get_top_window();

  frame = tool_widget_get("timezone_scrolledwindow");
  
    map = gnome_map_new ( image, mapwidth, mapheight, aa );
    if (!map) {
	g_warning ("Could not create map view.");
	exit (1);
    }

    setup_item(map->image_item);
    gtk_signal_connect (GTK_OBJECT (map->canvas), "event",
			(GtkSignalFunc) canvas_event,
			NULL);

    gtk_container_add (GTK_CONTAINER (frame), map->canvas);

    statusbar = tool_widget_get("timezone_status");
    gtk_statusbar_push (GTK_STATUSBAR (statusbar), 1, "Unselected");
    gtk_widget_show (statusbar);  /* TODO: Remove this line */

    viewcombo = create_view_menu ();

  gtk_container_add(GTK_CONTAINER(tool_widget_get("timezone_view_frame")),
                                  viewcombo);

  gtk_signal_connect(GTK_OBJECT(map->canvas), "size-allocate", resize_canvas, NULL);
  
    /* put cities on the world map */
    draw_cities (map);

    /* add list of all timezones */
    frame = gtk_frame_new (NULL);  /* TODO: Remove this line */
  
  gtk_container_add(GTK_CONTAINER(tool_widget_get("timezone_list_frame")),
			create_location_list (&locationlist));
